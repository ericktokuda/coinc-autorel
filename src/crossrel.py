...
def run_experiment(top, dfref, nreq, k, h, runid, mincompsz, coincexp, isext, outdir):
    ...
    means = np.zeros((n2, maxdist), dtype=float)
    stds = np.zeros((n2, maxdist), dtype=float)
...
def run_experiments(cfg, nprocs, outpath, outdir):
    ...
    dfref = pd.read_csv(cfg['refnodes'][0], sep='\t') # Just uses the first

    argsconcat = list(product(tops, [dfref], ns, ks,  hs, runids, [mincompsz], coincexp, [False], [outdir]))
    args_ = product(exts, [dfref], [-1], [-1],  hs, [0], [mincompsz], coincexp, [True], [outdir])
